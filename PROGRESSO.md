...
20 requisições/dia no Gemini é pouco: uma única rodada de matching para uma usuária já usa até 15 chamadas de `avaliacao_vaga` sozinha. Reordenei pra Groq primeiro nessa tarefa, mas numa segunda tentativa o Groq também recusou (98.351/100.000 tokens do dia já usados, batendo o teto no meio da primeira chamada). Testei um retry pontual ~10 min depois: uma chamada pequena passou, mas a rodada completa (que pesa mais) voltou a esgotar o restante quase na hora — ou seja, o "retry em Xmin" que a API sugere não significa que sobra orçamento pra uma rodada inteira, só pra uma chamadinha. O consumo de hoje foi inflado por ser dia de desenvolvimento (várias rodadas de teste em cascata nas Fases 2 e 3); em uso normal (uma rodada de matching por dia, por usuária) a conta deve fechar bem mais folgada. Reduzi a descrição da vaga embutida no prompt de 4000 para 2000 caracteres pra cada chamada custar menos token. Se mesmo assim apertar com as duas usuárias rodando cron diário, a próxima opção é somar um terceiro provedor gratuito da Seção 6.3 (OpenRouter, Cerebras ou Mistral) como segundo fallback — não fiz isso agora, sem sinal de que seja necessário no uso real.

### Pendente
- Rodar `rodar_matching()` do zero (banco já limpo, sem estado parcial) quando a cota realmente resetar — provavelmente preciso esperar o próximo dia, não só alguns minutos. Só depois disso revisar as 10 vagas melhor pontuadas como `validador` e fechar a Fase 3 de fato.
- Testar a Entrevista 2 de ponta a ponta com um gap `documentacao` real (mecânica já validada com dado sintético em `test_gaps.py`, mas não com uma pergunta real gerada a partir de um gap de verdade).

## Próximo passo
Esperar o reset diário da cota (não insisto mais sozinho hoje — duas tentativas já esgotaram de novo) e então rodar o matching completo com dado real pra fechar a Fase 3.
